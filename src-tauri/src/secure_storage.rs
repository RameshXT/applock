...
pub fn restore_from_backup(path: &Path) -> Result<(), StorageError> {
    ...
    if !bak_path.exists() {
        return Err(StorageError::BackupFailed("Backup file does not exist".to_string()));
    }
...
fn write_atomic(path: &Path, data: &[u8], is_encrypted: bool) -> Result<(), StorageError> {
    ...
    let verify_result = if is_encrypted {
        ...
        serde_json::from_str::<IntegrityWrapper<serde_json::Value>>(&content)
            .map(|wrapper| {
                let actual_check = hex::encode(crypto::calculate_checksum(serde_json::to_string(&wrapper.data).unwrap().as_bytes()));
...
pub fn read_encrypted_internal<T: DeserializeOwned>(app_handle: &AppHandle, filename: &str) -> Result<T, StorageError> {
    ...
    let decrypted = crypto::decrypt_with_integrity(&data)
        .map_err(|e| match e {
            crypto::CryptoError::IntegrityCheckFailed => StorageError::Tampered("Checksum mismatch".to_string()),
...
pub fn write_encrypted_internal<T: Serialize>(app_handle: &AppHandle, filename: &str, data: &T) -> Result<(), StorageError> {
    ...
    let encrypted = crypto::encrypt_with_integrity(&json).map_err(|e| StorageError::WriteFailure(e.to_string()))?;
...
pub fn read_json_internal<T: DeserializeOwned + Serialize>(app_handle: &AppHandle, filename: &str) -> Result<T, StorageError> {
    ...
    if actual_checksum != wrapper.checksum {
        return Err(StorageError::Tampered("Plaintext checksum mismatch".to_string()));
    }
...
pub async fn verify_storage_integrity_internal(app_handle: AppHandle) -> Result<bool, String> {
    ...
    for (file, is_enc) in files {
        let path = match resolve_path(&app_handle, file) {
            Ok(p) => p,
            Err(_) => { all_valid = false; continue; }
        };
        ...
        let valid = if is_enc {
            ...
            if let Ok(wrapper) = serde_json::from_str::<IntegrityWrapper<serde_json::Value>>(&content) {
...
pub async fn get_storage_status_internal(app_handle: AppHandle) -> Result<StorageStatus, String> {
    let creds = resolve_path(&app_handle, CREDENTIALS_FILE).map(|p| p.exists() && crypto::decrypt_with_integrity(&fs::read(p).unwrap_or_default()).is_ok()).unwrap_or(false);
    let settings = resolve_path(&app_handle, SETTINGS_FILE).map(|p| p.exists()).unwrap_or(false);
    let apps = resolve_path(&app_handle, LOCKED_APPS_FILE).map(|p| p.exists()).unwrap_or(false);
    let logs = resolve_path(&app_handle, LOGS_FILE).map(|p| p.exists()).unwrap_or(false);
