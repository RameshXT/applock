...
export function ConfirmModals({
  appName,
  appToRemove, setAppToRemove, onConfirmRemoval,
  appsToBulkUnlock, setAppsToBulkUnlock, onConfirmBulkUnlock,
  showResetConfirm, setShowResetConfirm,
  showResetFinal, setShowResetFinal,
  onReset,
}: ConfirmModalsProps) {
  return (
    <AnimatePresence>
      {appToRemove && (
        <motion.div initial={{ opacity: 0 }} animate={{ opacity: 1 }} className={styles.modalOverlay}>
          <motion.div initial={{ scale: 0.95 }} animate={{ scale: 1 }} className={styles.modalCard}>
            ...
            <p>Are you sure you want to unlock <strong>{appToRemove.name}</strong>? This application will no longer be protected by {appName}.</p>
            <div className={styles.modalActions}>
              <button className={styles.modalCancel} onClick={() => setAppToRemove(null)}>Cancel</button>
              <button className={styles.modalConfirm} onClick={onConfirmRemoval}>Remove Lock</button>
            </div>
          </motion.div>
        </motion.div>
      )}
        ...
        <motion.div initial={{ opacity: 0 }} animate={{ opacity: 1 }} className={styles.modalOverlay}>
          <motion.div initial={{ scale: 0.95 }} animate={{ scale: 1 }} className={styles.modalCard}>
            ...
            <div className={styles.modalActions}>
              ...
              <button className={styles.modalConfirm} onClick={onConfirmBulkUnlock}>Remove Locks</button>
            </div>
          </motion.div>
        </motion.div>
      )}
      ...
      {showResetConfirm && (
        <motion.div initial={{ opacity: 0 }} animate={{ opacity: 1 }} className={styles.modalOverlay}>
          <motion.div initial={{ scale: 0.95 }} animate={{ scale: 1 }} className={styles.modalCard}>
            ...
            <h3>Wipe All Data?</h3>
            <p>Are you sure you want to reset {appName}? This will remove all your protected apps and security settings.</p>
            <div className={styles.modalActions}>
        ...
        <motion.div initial={{ opacity: 0 }} animate={{ opacity: 1 }} className={styles.modalOverlay}>
          <motion.div initial={{ scale: 0.95 }} animate={{ scale: 1 }} className={styles.modalCard}>
            ...
            <div className={styles.modalActions}>
              ...
              <button className={styles.modalConfirm} onClick={onReset}>Reset Everything</button>
            </div>
          </motion.div>
        </motion.div>
      )}
